...
<body>
    <div class="container">
        ...
        <div class="latest-card" id="latestCard">
            ...
            <div class="latest-body" id="latestContent">
        ...
        <div class="hero">
            ...
            <div class="hero-badge">
                <span>📅 <span id="currentDate">2026-06-09</span></span>
                <span class="clock" id="liveClock">--:--:--</span>
                <span>⚡ 下一期: <span id="nextPeriod">2026149</span></span>
                <span>📊 基于 <span id="totalPeriods">180</span> 期历史数据</span>
                <span>🤖 AI验证已启用</span>
            </div>
        </div>
        ...
        <div id="tab1" class="tab-content active">
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-value" id="commonSum">14</div><div class="stat-label">最常见和值</div><div class="stat-desc">出现 28次</div></div>
                <div class="stat-card"><div class="stat-value" id="commonSpan">5</div><div class="stat-label">最常见跨度</div><div class="stat-desc">出现 32次</div></div>
                <div class="stat-card"><div class="stat-value" id="predictedPatternDisplay">组六</div><div class="stat-label">预测形态</div><div class="stat-desc">占比 68%</div></div>
                <div class="stat-card"><div class="stat-value" id="hitRateDisplay">26.7%</div><div class="stat-label">历史命中率</div><div class="stat-desc">最近30期命中8期</div></div>
            </div>
            <div class="glass-card">
                <div class="card-header"><span>📈</span><h2>走势图分析（点击图片可放大）</h2></div>
                <div class="card-body">
                    <div class="charts-grid">
                        <div class="chart-card"><h3>📈 和值走势图</h3><img src="fc3d_sum_trend.png" alt="和值走势图"></div>
                        <div class="chart-card"><h3>📐 跨度走势图</h3><img src="fc3d_span_trend.png" alt="跨度走势图"></div>
                        <div class="chart-card"><h3>📊 和值分布直方图</h3><img src="fc3d_sum_hist.png" alt="和值分布"></div>
                        <div class="chart-card"><h3>🥧 形态占比</h3><img src="fc3d_pattern_pie.png" alt="形态占比"></div>
                        <div class="chart-card"><h3>📍 位置频率分布</h3><img src="fc3d_position_freq.png" alt="位置频率"></div>
                        <div class="chart-card"><h3>🔢 奇偶走势图</h3><img src="fc3d_parity_trend.png" alt="奇偶走势"></div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <tr><td style="font-weight:600; color:#e2e8f0;">2026148</td><td style="color:#e2e8f0;">4 0 8</td><td style="color:#e2e8f0;">468, 462, 862, 482, 486</td><td class="hit">✅ 命中</td><td class="hit">第5注</td></tr>
                                <tr><td style="font-weight:600; color:#e2e8f0;">2026147</td><td style="color:#e2e8f0;">7 1 2</td><td style="color:#e2e8f0;">468, 462, 862, 482, 486</td><td class="miss">❌ 未命中</td><td>-</td></tr>
                                <tr><td style="font-weight:600; color:#e2e8f0;">2026146</td><td style="color:#e2e8f0;">4 6 4</td><td style="color:#e2e8f0;">468, 462, 862, 482, 486</td><td class="hit">✅ 命中</td><td class="hit">第1注</td></tr>
                                <tr><td style="font-weight:600; color:#e2e8f0;">2026145</td><td style="color:#e2e8f0;">2 7 9</td><td style="color:#e2e8f0;">468, 462, 862, 482, 486</td><td class="miss">❌ 未命中</td><td>-</td></tr>
                                <tr><td style="font-weight:600; color:#e2e8f0;">2026144</td><td style="color:#e2e8f0;">7 2 6</td><td style="color:#e2e8f0;">468, 462, 862, 482, 486</td><td class="hit">✅ 命中</td><td class="hit">第2注</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div class="glass-card">
                ...
                <div class="card-body">
                    ...
                    <div class="kill-stats">
                        <div class="kill-stat-card"><span style="font-size: 24px; color:#34d399;">6</span><br><span style="color:#cbd5e1;">总杀号数量</span></div>
                        <div class="kill-stat-card"><span style="font-size: 24px; color:#f59e0b;">85.3%</span><br><span style="color:#cbd5e1;">综合置信度</span></div>
                        <div class="kill-stat-card"><span style="font-size: 24px; color:#10b981;">🤖</span><br><span style="color:#cbd5e1;">AI验证模式</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card">
                ...
                <div class="card-body">
                    ...
                    <div class="method-card">
                        ...
                        <div class="method-detail">
                            <span>上期开奖: <span id="lastNumbersForKill">408</span> | 杀重号: 4,0,8</span>
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                ...
                                <tr><td class="badge-primary">低置信度</td><td class="text-gray">上期号码: <span id="lastNumbersForKillTable">408</span></td><td style="color:#f87171;">35%</td><td class="text-gray">谨慎杀号</td></tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <script>
        ...
        // 使用 500.com 的官方JSONP接口（更稳定）
        function fetchLatestLottery() {
            return new Promise((resolve, reject) => {
                ...
                window[callbackName] = function(data) {
                    ...
                    if (data && data.data && data.data.red) {
                        const red = data.data.red.split(',');
                        if (red.length >= 3) {
                            resolve([parseInt(red[0]), parseInt(red[1]), parseInt(red[2])]);
                        } else {
                            reject('数据格式错误');
                        }
                    } else {
                ...
                script.src = `https://www.500.com/api/proxy/lottery?lotteryId=1&action=latest&callback=${callbackName}`;
                script.onerror = () => {
                    delete window[callbackName];
                    document.body.removeChild(script);
                    reject('请求失败');
        ...
        // 备用数据
        const FALLBACK_NUMBERS = [4, 0, 8];

        // 更新页面上的最新开奖数据
        async function updateLatestLottery() {
            const latestContent = document.getElementById('latestContent');
            
            // 显示加载状态
            latestContent.innerHTML = '<div class="loading-spinner"></div><div>正在获取最新开奖数据...</div>';
            
            let numbers = null;
            try {
                numbers = await fetchLatestLottery();
            } catch (error) {
                console.log('API获取失败:', error);
            }
            ...
            if (!numbers) {
                ...
                latestContent.innerHTML = `
                    <div class="latest-numbers">
                        <div class="latest-ball">${numbers[0]}</div>
                        <div class="latest-ball">${numbers[1]}</div>
                        <div class="latest-ball">${numbers[2]}</div>
                    </div>
                    <div class="latest-info">
                        <span class="latest-info-item">🎨 形态: 组六</span>
                        <span class="latest-info-item">🔢 和值: 12</span>
                        <span class="latest-info-item">📏 跨度: 8</span>
                    </div>
                    <div class="refresh-time">⚠️ 使用备用数据，实时数据获取失败</div>
                `;
                return;
            }
            ...
            const isGroup3 = numbers[0] === numbers[1] || numbers[0] === numbers[2] || numbers[1] === numbers[2];
            const pattern = isGroup3 ? '组三' : '组六';
            const nextPeriodNum = parseInt(numbers.join('')) + 1;
            ...
            latestContent.innerHTML = `
                ...
                <div class="latest-info">
                    <span class="latest-info-item">🎨 形态: ${pattern}</span>
                    <span class="latest-info-item">🔢 和值: ${sum}</span>
                    <span class="latest-info-item">📏 跨度: ${span}</span>
                </div>
                <div class="refresh-time">✅ 数据实时获取自官方平台 | 打开即最新</div>
            `;
            
            // 更新头部信息
            document.getElementById('currentDate').innerText = new Date().toISOString().slice(0,10);
            document.getElementById('nextPeriod').innerText = `2026${nextPeriodNum}`;
            
            // 更新杀号系统中的上期号码
            const lastNumbersSpans = document.querySelectorAll('#lastNumbersForKill, #lastNumbersForKillTable');
            lastNumbersSpans.forEach(span => {
                if (span) span.innerText = `${numbers[0]}${numbers[1]}${numbers[2]}`;
            });
            
            // 更新推演结果
            updatePredictions(numbers);
        }
        
        // 基于最新开奖数据更新推演结果
        function updatePredictions(latestNumbers) {
            const predictionBody = document.getElementById('predictionBody');
            const numbers = latestNumbers;
            
            const hotCombos = [
                [4,6,8], [4,6,2], [8,6,2], [4,8,2], [4,8,6],
                [6,8,4], [2,6,8], [2,4,8], [6,4,2], [8,2,4]
            ];
            
            const candidates = [];
            let score = 16;
            
            for (const combo of hotCombos) {
                const numStr = `${combo[0]}${combo[1]}${combo[2]}`;
                if (numStr !== `${numbers[0]}${numbers[1]}${numbers[2]}`) {
                    const isGroup3 = combo[0] === combo[1] || combo[0] === combo[2] || combo[1] === combo[2];
                    candidates.push({
                        number: numStr,
                        score: score,
                        sum: combo[0] + combo[1] + combo[2],
                        pattern: isGroup3 ? '组三' : '组六',
                        aiScore: 92 - (candidates.length * 8)
                    });
                    score -= 1.5;
            ...
            const predictionHtml = `
                <div class="prediction-grid">
                    ${candidates.map((p, idx) => {
                        const nums = p.number.split('');
                        const scoreClass = idx === 0 ? 'score-high' : (idx < 3 ? 'score-mid' : 'score-low');
                        return `
                            <div class="prediction-item">
                                ...
                                <div><span class="prediction-score ${scoreClass}">${p.score}分</span></div>
                                <div class="ai-score">AI:${p.aiScore}分</div>
                                <div style="font-size: 11px; color:#94a3b8; margin-top: 5px;">形态:${p.pattern} | 和值:${p.sum}</div>
                            </div>
                        `;
                    }).join('')}
                </div>
                <div style="margin-top: 15px; padding: 10px; background: rgba(239,68,68,0.1); border-radius: 12px;">
                    <p style="color:#f87171; font-size: 12px;">⚠️ 已排除上期完全相同号码 ${numbers[0]}${numbers[1]}${numbers[2]}</p>
                </div>
            `;
            
            if (predictionBody) {
                predictionBody.innerHTML = predictionHtml;
            }
        }
        ...
        updateLatestLottery();
        // 每12小时刷新一次
        setInterval(updateLatestLottery, 12 * 60 * 60 * 1000);
    </script>
</body>
</html>
